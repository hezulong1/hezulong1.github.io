...
function createRouter(loadPageModule, fallbackComponent) {
  ...
  const router = {
    ...
    async go(href, options) {
      const { hash: hash2 } = new URL(href, fakeHost);
      const hasTextFragment = inBrowser && document.fragmentDirective && hash2.includes(":~:");
