...
import type { PostOutlineItem } from '@/app.d';
// import { useAside } from './aside';

import { getScrollOffset } from 'vitepress';
import { onMounted, onUnmounted, onUpdated } from 'vue';
import { throttleAndDebounce } from '../logic';
...
export function getHeaders(range: DefaultTheme.Config['outline']): PostOutlineItem[] {
  const headers = [
...
export function useActiveAnchor(container: Ref<HTMLElement>, marker: Ref<HTMLElement>): void {
  ...
  function activateLink(hash: string | null) {
    ...
    if (activeLink) {
      ...
      marker.value.style.top = activeLink.offsetTop + baseOffset + 'px';
      marker.value.style.opacity = '1';
    } else {
      marker.value.style.top = baseOffset + 'px';
