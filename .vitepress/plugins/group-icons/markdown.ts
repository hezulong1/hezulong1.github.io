...
export function groupIconMdPlugin(md: Markdown, options?: MdPluginOptions) {
  ...
  if (codeGroupOpenRule) {
    md.renderer.rules['container_code-group_open'] = (...args) => codeGroupOpenRule(...args).replace(
      labelRE,
  ...
  md.renderer.rules['container_code-group_open']! = (...args) => {
    ...
    return code.replace(
      ...
      (_, labelStart, beforeIcon, _icon, afterIcon, labelEnd) => `${labelStart}${beforeIcon.trim()}${afterIcon}${labelEnd}`,
    );
  };
  ...
  if (fenceRule) {
    md.renderer.rules.fence = (...args) => {
      ...
      if (!isOnCodeGroup && title && (!(token as any).src || isIncludedSnippet)) {
        ...
        return `<div class="vp-code-group vp-code-block">
      <div class="tabs">
        <label data-title="${md.utils.escapeHtml(title[1])}">${namedIconMatch ? title[1].replace(namedIconMatch[0], '') : title[1]}</label>
      </div>
      <div class="blocks">
        ${fenceRule(...args)}
      </div></div>
