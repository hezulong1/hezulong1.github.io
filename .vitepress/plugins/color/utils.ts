...
const ESCAPE_RE = /[$()*+.?[\\\]^{|}]/g;
export const escape = (str: string) => str.replace(ESCAPE_RE, '\\$&');
...
export function getLuminance(color?: string | null) {
  if (!color || color === 'transparent') return 1;

  // 如果是颜色名称，转换为十六进制
  let hex = color.toLowerCase();
  if (hex in presetColors && Object.hasOwn(presetColors, hex)) {
    hex = presetColors[hex as keyof typeof presetColors];
  }
...
export function isValidColor(color: ColorTypes): boolean {
  try {
